...
public final class GedProperties implements PropertiesAdapter {
    ...
    private void $initGlobalProperties$(final String xmlFilename) throws IOException {
        ...
        final var propertiesFilename = GedUtil.replaceFileExtension(xmlFilename, "properties");

        // ged.properties provider
        final var file = new File(mBasedir, propertiesFilename);
        if (file.exists()) {
    ...
    private InputStream getPropertiesStream(final String filename) throws IOException, PropertiesException {
        InputStream result;
        final var file = Paths.get(mBasedir, filename);
    ...
    public void init(final String basedir, final String filename) throws PropertiesException {
        ...
        try {
            ...
            final var workingFilename = computeFilename(filename);
            ...
            final var contextPackage = com.arkham.ged.properties.Root.class.getPackage().getName();
            LOGGER.info("init() : unmarshalling configuration={}", contextPackage);
            final var root = (Root) JaxbAdapter.unmarshall(is, contextPackage);
            ...
            final var ds = root.getDatasource();
            ...
            final var fkpTypeList = root.getFileKeyProvider();
            ...
            if (mStorageType.getBaseDir() != null) {
                final var baseDir = pt.translate(mStorageType.getBaseDir());
                final var baseDirFile = new File(baseDir);
    ...
    public String getMatchingTypmed(final String filename) {
        var ext = GedUtil.getFileExtension(filename);
        if (ext != null) {
            ext = ext.toLowerCase();
            final var mt = getMedia();
            // Cannot be null because of default value in XSD
            final var defaultTypmed = mt.getDefault();

            final var typmed = getMediaMapping().get(ext);
            if (typmed == null) {
    ...
    public static int getMaxSize(final List<OptionalParameterType> optl) {
        var result = GedProperties.DEFAULT_INTEGRATE_MAX_SIZE;
        final var maxSizeOpt = getOptionalParameters(optl, GedProperties.PARAM_INTEGRATE_MAX_SIZE);
        if (maxSizeOpt != null) {
            try {
    ...
    private static void checkStorageBaseDir(final File baseDir) throws PropertiesException {
        try {
            ...
            // Netherless : I do it !
            final var localhost = System.getProperty("COMPUTERNAME", "unknown");
            final var currentClassLoader = CurrentClassLoader.getCurrentClass().toString();
            final var timestamp = String.valueOf(System.nanoTime());
            final var md5 = new GxMD5(localhost + "-" + currentClassLoader + "-" + timestamp);

            final var filename = ".ged_storage_verifyer-" + md5.getStringDigest();

            final var test = new File(baseDir, filename);
            var ok = true;
            if (test.exists()) {
                ok = test.delete();
            }
    ...
    public boolean isOptionalParameterActive(final OptionalParameterType opt, final File file) {
        if (opt != null) {
            ...
            final var expression = opt.getExpression();
            if (expression != null && expression.trim().length() > 0) {
                final var evp = new GedExprValueProvider(file);
                final var es = new ExprSolver(evp);
                try {
                    final var result = es.solve(expression);
