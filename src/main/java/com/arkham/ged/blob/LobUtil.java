...
public final class LobUtil {
    ...
    public static int getCodsocPhy(final Connection con, final int codsoc, final String codent, final String segment) throws DocumentLinkException {
        ...
        try {
            ...
            final var hc = new HashCoder(codsoc, codent, segment);
            final var res = mCodsocCache.get(hc);
            if (res != null) {
                return res.intValue();
            }

            try (var psMev = con.prepareStatement("SELECT CODSOC_PHY FROM MEV WHERE CODSOC = ? AND CODENT = ? AND SEGMENT = ?")) {
                psMev.setInt(1, codsoc);
                psMev.setString(2, codent);
                psMev.setString(3, segment);

                try (var rs = psMev.executeQuery()) {
                    if (rs.next()) {
                        final var codsocphy = rs.getInt(1);
    ...
    static void adaptCodsocPhy(final Connection con, final DocumentLinkBean bean) throws DocumentLinkException {
        if (bean.getCodsocPhy() == -1) {
            final var codsocPhy = getCodsocPhy(con, bean);
