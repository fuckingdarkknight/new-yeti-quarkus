...
public class DocumentLinkBean extends DocumentLink implements Cloneable {
    ...
    public void updateEventKey() {
        final var nomcle = getKeydoc();
        // NUMEVE is 9 bytes length
        if (nomcle != null && "EVE".equals(getEnttyp()) && nomcle.length() >= 1 + 3 + 9) {
            final var achvteValue = nomcle.substring(0, 1);
            final var typeveValue = nomcle.substring(1, 4);
            final var numeveValue = GedUtil.getInt(nomcle.substring(4, 13), -1);
            if (numeveValue > -1) {
            ...
            if (nomcle.length() > 1 + 3 + 9) {
                final var numposValue = GedUtil.getInt(nomcle.substring(13), -1);
                if (numposValue > -1) {
    ...
    public void updateThirdKey() {
        ...
        if (nomcle != null && "TIE".equals(getEnttyp()) && nomcle.length() > 3) {
            final var typtieValue = nomcle.substring(0, 3);
            final var sigtieValue = nomcle.substring(3);
    ...
    public static DocumentLinkBean unserialize(byte[] obj) throws DocumentLinkException {
        ...
        try (var bais = new ByteArrayInputStream(obj)) {
            try (var ois = new ObjectInputStream(bais)) {
                final var result = ois.readObject();
    ...
    public String getCoddoc() {
        final var result = super.getCoddoc();
