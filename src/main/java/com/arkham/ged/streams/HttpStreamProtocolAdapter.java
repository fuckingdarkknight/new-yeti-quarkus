...
public class HttpStreamProtocolAdapter extends AbstractStreamProtocolAdapter<String, Map<String, Object>> {
	...
	public InputStream getStream() throws StreamProtocolException {
		final HttpClientBuilder builder = getClientBuilder();
		...
		try (CloseableHttpClient client = builder.setDefaultCookieStore(cookieStore).setDefaultHeaders(defaultHeaders).build()) {
			...
			final byte[] result = execute(client, post, true);
	...
	protected HttpClientBuilder getClientBuilder() throws StreamProtocolException {
		try {
			...
			final SSLConnectionSocketFactory sslSocketFactory = new SSLConnectionSocketFactory(sslContext, new DefaultHostnameVerifier());
			final HttpClientBuilder builder = HttpClients.custom().setSSLSocketFactory(sslSocketFactory);
	...
	private static String getCharset(String contentType) {
		...
		final int pos = contentType.toUpperCase().indexOf("CHARSET=");
		if (pos > -1) {
	...
	private static String getFilename(String contentDisposition) {
		...
		final int pos = contentDisposition.toUpperCase().indexOf("FILENAME=");
		if (pos > -1) {
			String s = contentDisposition.substring(pos + 9);
	...
	private List<NameValuePair> getPostParams() {
		...
		final Map<String, Object> o = getOptions();
		if (o != null) {
