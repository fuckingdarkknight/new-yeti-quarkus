...
public final class StreamProtocolFactory {
	...
	public static StreamProtocolAdapter create(String filepath, Connection con, Object o) throws StreamProtocolException {
		final GED_PROTOCOL scheme = GED_PROTOCOL.getScheme(filepath);
		final String value = getValue(filepath);
		AbstractStreamProtocolAdapter result;

		switch (scheme) {
			case FILE:
				result = new FileStreamProtocolAdapter(value);
				break;
			case URL:
				result = new HttpStreamProtocolAdapter(value);
				break;
			default:
				result = null;
				break;
