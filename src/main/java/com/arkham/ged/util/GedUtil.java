...
public final class GedUtil {
    ...
    public static String[] splitValue(final String value, final String delim) {
        if (value != null) {
            final List<String> result = new ArrayList<>();
            final var st = new StringTokenizer(value, delim, false);
    ...
    public static int[] splitIntValue(final String value, final String delim) {
        final var s = splitValue(value, delim);
    ...
    public static String[] splitValues(final String value) {
        if (value != null) {
            final var result = SPLIT_SEQ.split(value);
    ...
    public static String getFileExtension(final String filename) {
        ...
        if (dotPos != -1) {
            final var result = filename.substring(dotPos + 1);
    ...
    public static String replaceFileExtension(final String filename, final String extension) {
        ...
        final var dotPos = filename.lastIndexOf('.');
        if (dotPos != -1) {
            final var result = filename.substring(0, dotPos);
            if (result.trim().length() == 0) {
                return null;
            }
    ...
    public static void copyFile(final File source, final File target, final boolean nio) throws IOException {
        if (nio) {
            ...
            try (var is = Files.newInputStream(source.toPath()); var os = Files.newOutputStream(target.toPath())) {
    ...
    public static void initMDC() {
        ...
        final var gedNodeName = System.getProperty(NODE_INSTANCE_PROPERTY, "undefined");
    ...
    public static String convertYamlToJson(final String message) throws IOException {
        ...
        final var yamlObject = yamlReader.readValue(message, Object.class);
    ...
    public static String convertJsonToYaml(final String message) throws IOException {
        final var jsonNodeTree = new ObjectMapper().readTree(message);
