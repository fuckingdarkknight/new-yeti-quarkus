...
public class AddDimensionedImage {
    ...
    /**
     * Constructor AddDimensionedImage
     *
     ...
     * @param imageFormat eg.: Workbook#PICTURE_TYPE_PNG
    ...
    public void addImageToSheet(final String cellNumber, final InputStream imageFile, final double reqImageWidthMM, final double reqImageHeightMM, final ImageBehaviorType resizeBehaviour, final int imageFormat) throws IOException {
        ...
        final var cellRef = new CellReference(cellNumber);
        addImageToSheet(cellRef.getCol(), cellRef.getRow(), imageFile, reqImageWidthMM, reqImageHeightMM, resizeBehaviour, imageFormat);
    }
    ...
    public void addImageToSheet(final int colNumber, final int rowNumber, final InputStream imageFile, final double reqImageWidthMM, final double reqImageHeightMM, final ImageBehaviorType resizeBehaviour, final int imageFormat) throws IOException {
        ...
        if (resizeBehaviour != ImageBehaviorType.EXPAND_COL && resizeBehaviour != ImageBehaviorType.EXPAND_ROW && resizeBehaviour != ImageBehaviorType.INCELL && resizeBehaviour != ImageBehaviorType.OVERLAY) {
        ...
        colClientAnchorDetail = fitImageToColumns(colNumber, reqImageWidthMM, resizeBehaviour);
        rowClientAnchorDetail = fitImageToRows(rowNumber, reqImageHeightMM, resizeBehaviour);
        ...
        try (var os = new ByteArrayOutputStream()) {
            ...
            final var index = mSheet.getWorkbook().addPicture(os.toByteArray(), imageFormat);
    ...
    private ClientAnchorDetail fitImageToColumns(final int colNumber, final double reqImageWidthMM, final ImageBehaviorType resizeBehaviour) {
        ...
        colWidthMM = ConvertImageUnits.widthUnits2Millimetres((short) mSheet.getColumnWidth(colNumber));
        ...
        if (colWidthMM < reqImageWidthMM) {
            ...
            if (resizeBehaviour == ImageBehaviorType.EXPAND_COL || resizeBehaviour == ImageBehaviorType.INCELL) {
                // Set the width of the column by converting the required image
                // width from millimetres into Excel's column width units.
                mSheet.setColumnWidth(colNumber, ConvertImageUnits.millimetres2WidthUnits(reqImageWidthMM));
            ...
            } else if (resizeBehaviour == ImageBehaviorType.OVERLAY || resizeBehaviour == ImageBehaviorType.EXPAND_ROW) {
                ...
                colClientAnchorDetail = calculateColumnLocation(colNumber, reqImageWidthMM);
            }
        } else {
    ...
    private ClientAnchorDetail fitImageToRows(final int rowNumber, final double reqImageHeightMM, final ImageBehaviorType resizeBehaviour) {
        ...
        var row = mSheet.getRow(rowNumber);
        if (row == null) {
        ...
        var rowHeightMM = row.getHeightInPoints() / ConvertImageUnits.POINTS_PER_MILLIMETRE;
        ...
        if (rowHeightMM < reqImageHeightMM) {
            if (resizeBehaviour == ImageBehaviorType.EXPAND_ROW || resizeBehaviour == ImageBehaviorType.INCELL) {
            ...
            } else if (resizeBehaviour == ImageBehaviorType.OVERLAY || resizeBehaviour == ImageBehaviorType.EXPAND_COL) {
                // If the user has chosen to overlay both rows and columns or just
                // to expand ONLY the size of the columns, then calculate how to lay
                // the image out ver one or more rows.

                rowClientAnchorDetail = calculateRowLocation(rowNumber, reqImageHeightMM);
            }
        } else {
    ...
    private ClientAnchorDetail calculateColumnLocation(final int startingColumn, final double reqImageWidthMM) {
        ...
        var totalWidthMM = 0.0D;
        var colWidthMM = 0.0D;
        double overlapMM;
        double coordinatePositionsPerMM;
        var toColumn = startingColumn;
        int inset;

        // Calculate how many columns the image will have to
        // span in order to be presented at the required size.
        while (totalWidthMM < reqImageWidthMM) {
            colWidthMM = ConvertImageUnits.widthUnits2Millimetres((short) mSheet.getColumnWidth(toColumn));
    ...
    public static class ConvertImageUnits {
        ...
        public static int widthUnits2Pixel(final short widthUnits) {
            var pixels = widthUnits / EXCEL_COLUMN_WIDTH_FACTOR * UNIT_OFFSET_LENGTH;
            final var offsetWidthUnits = widthUnits % EXCEL_COLUMN_WIDTH_FACTOR;
