...
public class ExcelUtil {
	...
	public static void copyRow(final Workbook workbook, final Sheet worksheet, final int from, final int to) {
		...
		final Row sourceRow = worksheet.getRow(from);
		if (sourceRow == null) {
	...
	private static void copyCellFormula(final Workbook workbook, final Sheet sheet, final Cell sourceCell, final Cell targetCell) {
		...
		if (cellType == CellType.FORMULA && sourceCell.getCellFormula() != null) {
			...
			final SharedFormula sharedFormula = new SharedFormula(SpreadsheetVersion.EXCEL2007);
			final Ptg[] sharedFormulaPtg = FormulaParser.parse(sourceCell.getCellFormula(), formulaParsingWorkbook, FormulaType.CELL, workbook.getSheetIndex(sheet));

			final int shiftFormula = targetCell.getRowIndex() - sourceCell.getRowIndex();
			final Ptg[] convertedFormulaPtg = sharedFormula.convertSharedFormulas(sharedFormulaPtg, shiftFormula, 0);
	...
	private static void copyMergeRegion(final Sheet worksheet, final Row sourceRow, final Row newRow, final CellRangeAddress mergedRegion) {
		...
		if (range.getFirstRow() == sourceRow.getRowNum()) {
			...
			final CellRangeAddress newCellRangeAddress = new CellRangeAddress(newRow.getRowNum(), lastRow, range.getFirstColumn(), range.getLastColumn());
