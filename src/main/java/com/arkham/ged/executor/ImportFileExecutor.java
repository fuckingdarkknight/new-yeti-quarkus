...
public abstract class ImportFileExecutor extends AbstractScanFileExecutor<InputScanFileDef> {
    ...
    protected void integrate(final File sourceFile, final File integrate, final FileKey fk) throws ExecutorException {
        if (integrate.exists()) {
            ...
            final var opt = getPA().getOptionalParameter(getSFD().getParam(), GedProperties.PARAM_INTEGRATE_MAX_SIZE);
            if (opt != null) {
                final var maxFileSize = GedProperties.getMaxSize(getSFD().getParam());
            ...
            try {
                ...
                final var aal = getActionList();
                for (final AbstractAction aa : aal) {
                    final var oet = aa.getActionType().getOnError();
    ...
    protected void renameOrDeleteFile(final File sourceFile, final File integrate, final FileKey fk) throws IOException {
        if (fk != null) {
            var fileRefRename = getSFD().getFileRefRename();
            ...
            if (fk.isRefFile()) {
                ...
                if (getSFD().isFileRefDeletion() && sourceFile != null) {
                    ...
                    final var renamed = new File(GedUtil.replaceFileExtension(sourceFile.getCanonicalPath(), fileRefRename));
                ...
                if (fileRefRename != null) {
                    if (getSFD().isFileDeletion() && integrate != null && integrate.delete()) {
                        ...
                        final var renamed = new File(GedUtil.replaceFileExtension(integrate.getCanonicalPath(), fileRefRename)); // NOSONAR
                        if (renamed.delete()) {
