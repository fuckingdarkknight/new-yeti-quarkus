...
public class MailUtil {
    ...
    public static File createZip(Map<String, String> files, String zipName, Connection con) throws ActionException {
        try {
            final var outputPath = Files.createTempFile(zipName + "-", ".zip");

            final var outputFile = outputPath.toFile();

            try (var os = new ZipOutputStream(Files.newOutputStream(outputFile.toPath()))) {
    ...
    private static void addZipEntry(String key, Connection con, Entry<String, String> entry, ZipOutputStream os, int index) throws IOException {
        try (final var spa = StreamProtocolFactory.create(key.trim(), con)) {
            // First get the stream to trigger additionals step
            @SuppressWarnings("resource")
            final var is = spa.getStream();
            if (is != null) {
                ...
                final var zipEntry = new ZipEntry(index + "-" + name);
