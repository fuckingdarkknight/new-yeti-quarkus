	...
	public FileKey getKey(File file, Connection con, PropertiesAdapter pa, List<OptionalParameterType> opt) throws FileKeyProviderException {
		...
		if (dotPos != -1) {
			try {
				...
				final String type = fn.substring(0, 3);
				final DirectIntegMode dim = DirectIntegMode.valueOf(type);
				final String[] keys = GedUtil.splitKey(fn, dim.getSplit());

				final String typtie = keys[0];
				final int codsoc = Integer.parseInt(keys[1]);
				// La clé est la concaténation du reste des valeurs disponibles
				final StringBuilder nomcle = new StringBuilder();
