...
public class FlatFileKeyProvider extends FileKeyProvider {
    ...
    protected FileKey getFileKey(FlatProp p) throws FileKeyProviderException {
        // Define the PK
        final var codsoc = p.getProperty("codsoc");
        var entity = -1;
        try {
        ...
        final var nomcle = p.getProperty("nomcle"); // the key (match to NOMCLE)
        ...
        // Others fields
        final var lib256 = p.getProperty("lib256");
        final var typmed = p.getProperty("typmed");
        final var codlan = p.getProperty("codlan");
        final var uticod = p.getProperty("uticod");
        final var updateMode = p.getProperty("updateMode");

        final var fk = new FileKey(entity, typtie, nomcle, lib256, typmed, codlan, uticod, filename, updateMode, p);
